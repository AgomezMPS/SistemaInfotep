...
| Subtítulos | `subtitles1.srt` … `subtitles20.srt` en la raíz del proyecto (un archivo por clase). |
| Generación automática | Script `transcribe-subtitles-4-20.ps1` y registro `transcribe-subtitles-log.txt` (histórico de ejecución). |
...
> Los **vídeos `.mp4`** no se incluyen en Git por tamaño; la carpeta de lecciones conserva un `README.md` para uso local. Los subtítulos `.srt` sí van en el repositorio.

---

## Verificación de subtítulos (estado actual)

Revisión realizada sobre los archivos presentes en disco: formato **SRT válido** (bloques numerados, marcas de tiempo `HH:MM:SS,mmm --> …`, texto en español legible). Hay **20 archivos** `subtitlesN.srt` con métricas de tamaño y número de cuadros comprobados en disco.
...
Cada parte corresponde al vídeo homónimo en la carpeta de materiales. El subtítulo asociado es `subtitles{n}.srt` (disponible para **n = 1 … 20**).
...
- **Subtítulos:** `subtitles4.srt` (Whisper)
...
- **Subtítulos:** `subtitles5.srt` (Whisper)
...
- **Subtítulos:** `subtitles6.srt` (Whisper)
...
- **Subtítulos:** `subtitles7.srt` (Whisper)
...
- **Subtítulos:** `subtitles8.srt` (Whisper)
...
- **Subtítulos:** `subtitles9.srt` (Whisper)
...
- **Subtítulos:** `subtitles10.srt` (Whisper)
...
- **Subtítulos:** `subtitles11.srt` (Whisper)
...
- **Subtítulos:** `subtitles12.srt` (Whisper)
...
- **Subtítulos:** `subtitles13.srt` (Whisper)
...
- **Subtítulos:** `subtitles14.srt` (Whisper)
...
- **Subtítulos:** `subtitles15.srt` (Whisper)
...
- **Subtítulos:** `subtitles16.srt` (Whisper)
...
- **Subtítulos:** `subtitles17.srt` (Whisper)
...
- **Subtítulos:** `subtitles18.srt` (Whisper)
...
- **Subtítulos:** `subtitles19.srt` (Whisper)
...
- **Subtítulos:** `subtitles20.srt` (Whisper)

---

## Regenerar subtítulos (opcional)

Si borras algún `subtitlesN.srt` (por ejemplo para volver a transcribir con otro modelo de Whisper) o añades nuevos vídeos, puedes ejecutar de nuevo el script desde PowerShell:

```powershell
Set-Location -LiteralPath "ruta\al\proyecto"
